...
export class Orchestrator {
  ...
  _updateMemeticField() {
    ...
    const stanceCounts = {}
    stances.forEach(stance => { stanceCounts[stance] = (stanceCounts[stance] || 0) + 1 })
    const dominant = Object.entries(stanceCounts).reduce((a, b) => a[1] > b[1] ? a : b)
    const totalEnergy = this.agents.reduce((sum, a) => sum + a.state.energy, 0)
    const avgPlasticity = this.agents.reduce((sum, a) => sum + a.state.plasticity, 0) / this.agents.length
    this.memeticField = {
      dominantStance: dominant[0],
      strength: (dominant[1] / this.agents.length) * (totalEnergy / this.agents.length),
      coherence: avgPlasticity * 0.7 + (dominant[1] / this.agents.length) * 0.3,
      lastUpdate: this.t
    }
  }

  _computeMemeticInfluence(speaker, utterance, receiver) {
    const baseInfluence = computeMemeticInfluence(speaker, utterance, receiver)
    const fieldStrength = this.memeticField.strength * (speaker.stance === this.memeticField.dominantStance ? 1.2 : 0.8)
    return {
      ...baseInfluence,
      strength: clamp(baseInfluence.strength * 0.7 + fieldStrength * 0.3),
